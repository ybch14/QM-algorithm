...
int main()
{
	...
	short int graph[1025],tempgraph[1025];
	//2为无用项，0为被覆盖，1为未被覆盖
	...
	for(i=0;i<nq;i++)
	{
		...
		//}
		memset(graph,0,sizeof(short)*1025);
		memset(tempgraph,0,sizeof(short)*1025);
		for(j=0;j<nm[i];j++)
			graph[min[i][j]]=1;
		tempsize1=print1.size();
		for(j=0;j<tempsize1;j++)
		...
		if(sum!=0)
		{
			...
			else if(queue.empty())
			{
				...
				for(ii=2;ii<=nature.size();ii++)//尝试覆盖的项数
				{
					...
					{
						maxcover=0;cover=false;memcpy(tempgraph,graph,1025*sizeof(short int));
						for(k=0;k<queue[j].source.size();k++)
						{
							for(m=0;m<nature[queue[j].source[k]]->value.size();m++)
							{
								if(tempgraph[nature[queue[j].source[k]]->value[m]]==1)
								{
									tempgraph[nature[queue[j].source[k]]->value[m]]=0;
									maxcover++;
								}
							}
						}
						for(m=0;m<nature[queue[j].itself]->value.size();m++)
						{
							if(tempgraph[nature[queue[j].itself]->value[m]]==1)
							{
								tempgraph[nature[queue[j].itself]->value[m]]=0;
								maxcover++;
							}
						}
